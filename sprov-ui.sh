...
#   Github - sprov-ui: https://github.com/mikewubox/sprov-ui
#======================================================
...
install() {
    install_base
    bash <(curl -L -s https://github.com/ttys0001/sprov-ui/raw/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/mikewubox/sprov-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/ttys0001/sprov-ui/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/ttys0001/sprov-ui/raw/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
